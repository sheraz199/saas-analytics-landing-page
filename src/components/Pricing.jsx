...
const Pricing = () => {
  ...
  const plans = [
    { name: "Starter", description: "Perfect for small projects and solo developers.", priceMonthly: 19, priceYearly: 15, features: ["Up to 5 projects", "Real-time analytics", "24-hr retention", "Community support"], isPopular: false },
    { name: "Pro", description: "Engineered for growing teams and scaling startups.", priceMonthly: 49, priceYearly: 39, features: ["Unlimited projects", "Advanced AI analytics", "30-day retention", "Priority 24/7 support", "API integration"], isPopular: true },
    { name: "Enterprise", description: "For large-scale operations requiring max power.", priceMonthly: 99, priceYearly: 79, features: ["Custom infrastructure", "Dedicated AI models", "Unlimited retention", "Dedicated manager", "SLA guarantees"], isPopular: false }
  ];
  ...
  const handleScroll = () => {
    if (scrollContainerRef.current) {
      ...
      setActiveCardIndex(Math.round(scrollPosition / cardWidth));
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-6 md:mb-8">
          ...
          <div className="mt-8 flex items-center justify-center space-x-4">
            ...
            <button onClick={() => setIsYearly(!isYearly)} className="w-14 h-8 bg-slate-800 rounded-full p-1 transition-colors duration-300 relative focus:outline-none border border-slate-700">
        ...
        */}
          ...
          {plans.map((plan, index) => (
            <div 
              ...
              className={`min-w-[85vw] sm:min-w-[320px] md:min-w-0 snap-center shrink-0 bg-slate-900/40 border rounded-3xl p-6 md:p-8 flex flex-col justify-between transition-all duration-300 relative ${
                plan.isPopular ? 'border-emerald-500 shadow-[0_0_40px_rgba(16,185,129,0.2)] md:scale-105 z-10' : 'border-slate-800 hover:border-slate-700'
              }`}
            >
              {plan.isPopular && (
                <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-emerald-500 text-slate-950 text-[10px] md:text-xs font-black px-3 md:px-5 py-1.5 rounded-full uppercase tracking-wider">
                  Most Popular
                </span>
              )}
              ...
              <button className={`w-full py-3 md:py-4 px-4 rounded-full text-xs md:text-sm font-bold transition-all duration-200 ${plan.isPopular ? 'bg-emerald-500 text-slate-950 hover:bg-emerald-400' : 'bg-slate-800 text-white hover:bg-slate-700'}`}>
                Choose {plan.name}
              </button>
            </div>
          ))}
        ...
        <div className="flex md:hidden justify-center items-center space-x-3 mt-4">
          {plans.map((_, idx) => (
            <button 
              key={idx} onClick={() => scrollToCard(idx)}
              className={`h-2.5 rounded-full transition-all duration-300 cursor-pointer focus:outline-none ${activeCardIndex === idx ? 'w-8 bg-emerald-500' : 'w-2.5 bg-slate-700 hover:bg-slate-600'}`}
            ></button>
          ))}
        </div>
